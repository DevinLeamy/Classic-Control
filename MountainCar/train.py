...
class DQN():
    def __init__(self, states_cnt, hidden_cnt, actions_cnt, lr=0.0001, gamma=0.99, batch_sz=64, memory_sz=100000, update_fq=200):
    ...
    def store_model(self):
        torch.save(self.q_net.state_dict(), "models/MountainCarModel.pt")
        print("Model Saved")
...
def train(problem, episodes, max_iterations, gamma, epsilon, epsilon_decay, lr, threshold_value):
    ...
    for episode in range(episodes):
        ...
        for _iter in range(max_iterations):
            ...
            # Use custom reward (improves rate of improvement?)
            ...
            records.append([episode, _iter + 1, rewards, max_pos])
            if done or _iter == max_iterations - 1:
                print(episode, rewards, max_pos)
                break
...
records = train("MountainCar-v0", 2000, 200, 0.9, 1.0, 0.9, 0.001, -100)
for episode, iteration, rewards, max_pos in records:
    print("Episode: {}\tIterations: {}\tRewards: {}\t Max Position: {}".format(episode, iteration, rewards, max_pos))
